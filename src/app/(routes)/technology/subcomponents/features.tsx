...
export const Features = () => {
    ...
    const FeatureBtn = ({ id, text, icon }: { id: string; text: string; icon: React.JSX.Element }) => {
        return (
            <button
            className={`${ activeBtn === id ? 'bg-[rgba(0,168,73,0.1)] text-[rgba(0,168,73,1)] px-8' : 'text-[rgba(15,23,42,1)] px-4' } flex gap-2 items-center text-lg text-start transition-all duration-300 rounded-tl-2xl rounded-br-2xl py-4`}
            onClick={ () => handleFeature(id) }
            >
                <div className={`${ activeBtn === id ? 'bg-[rgba(0,168,73,1)]' : 'bg-[rgba(243,244,246,1)]' } p-3 rounded-2xl`}>
                    { icon }
                </div>
                { text }
            </button>
    ...
    return (
        <section className="mb-40">
            <div className="w-10/12 mx-auto">
                ...
                <div className="flex-1 flex justify-between items-center gap-2 pe-10">
                    ...
                    <div className="feature-border relative shrink-0 w-1/2 max-w-[500px] h-fit">
                        <div className="relative rounded-tl-[40px] md:rounded-tl-[125px] rounded-br-[40px] md:rounded-br-[125px] overflow-hidden z-50">
                            <Image
                            className="w-full object-cover scale-150 md:scale-100 min-h-[370px] md:h-[450px] max-h-[450px] rounded-tl-[40px] md:rounded-tl-[125px] rounded-br-[40px] md:rounded-br-[125px] border-b-8 border-r-8 border-[rgba(245,126,37,1)]"
                            src="/landing/hero.jpg"
                            alt="SFC"
                            width={100}
                            height={60}
                            />
                            <div className="absolute bottom-2 right-2 w-full h-fit flex flex-col gap-2 bg-gradient-to-t from-black to-black/0 text-white rounded-br-[40px] md:rounded-br-[125px] font-normal ps-8 pe-8 pb-4">
                        ...
                        <div className="feature-border-1 absolute inset-0 w-full h-full rounded-tl-[125px] rounded-br-[125px] bg-[rgba(0,168,73,0.1)] z-20"></div>
                        <div className="feature-border-2 absolute inset-0 w-full h-full rounded-tl-[125px] rounded-br-[125px] bg-[rgba(0,168,73,0.3)] z-30"></div>
                        <div className="feature-border-3 absolute inset-0 w-full h-full rounded-tl-[125px] rounded-br-[125px] bg-[rgba(0,168,73,0.5)] z-40"></div>
                    </div>
                </div>
            </div>
        </section>
