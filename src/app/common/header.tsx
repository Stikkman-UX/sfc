...
export const Header = () => {
    ...
    return (
        <nav className="fixed top-0 w-full bg-white/60 backdrop-blur-[35px] py-8 md:py-4 text-[15px] z-[999]">
                ...
                <div className="hidden md:flex md:gap-6 lg:gap-10 header-items">
                    {navItems.map(item => (
                        <Link
                            key={item.path}
                            href={item.path}
                            className={`${pathname === item.path ? 'text-[#00A849] font-bold' : 'font-medium'}`}
                        >
                            {item.label}
                        </Link>
                    ))}
